...
static int file_i2c;   // The i2c location in the device
static unsigned char i2c_buffer[BUFFER_SIZE] = {0};  // buffer to store data from slave chip
static int i2c_buffer_index = 0;
...
int connect_i2c(){
  ...
  if(access(filename, F_OK) == -1){
    printf("/dev/i2c-1 does not exists\n");
    return 1;
  }
  if ((file_i2c = open(filename,O_RDWR )) <0){ 
   ...
   printf("Success in opening I2C bus\n");
  return 0;
}
...
/*
 Get the value at the slave chip register
    ...
    Returns:: the index of the buffer where the value is stored. 
 */
int i2c_get_register_val(unsigned char reg){
  ...
  int BYTE_SIZE = 1;
  int read_status = i2c_read_bytes(BYTE_SIZE);
  ...
  int val = i2c_buffer_index-BYTE_SIZE;
  return val;
}
...
/*
 Set the value of the register in the slave chip
    ...
    Returns:: write_status: 1 if succeed, else returns 0
 */
...
/*
Get the value at the buffer. 

Parameters:: The index of the buffer you want to accuess
 */
